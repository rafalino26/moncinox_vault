    ...
    return (
        <div className="bg-white/30 p-4 rounded-2xl shadow-lg">
            <div className="flex justify-center mb-4 border-b border-black/10">
                <button onClick={() => setView('tabel')} className={`px-4 py-2 font-semibold transition-colors rounded-t-lg ${view === 'tabel' ? 'border-b-2 border-[#743ab7] text-[#743ab7]' : 'text-slate-500'}`}>inii tabell</button>
                <button onClick={() => setView('grafik')} className={`px-4 py-2 font-semibold transition-colors rounded-t-lg ${view === 'grafik' ? 'border-b-2 border-[#743ab7] text-[#743ab7]' : 'text-slate-500'}`}>inii grafikk</button>
            </div>
